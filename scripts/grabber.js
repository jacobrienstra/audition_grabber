...
var others = ["PREPARATION", "SEEKING", "CONTRACT", "PERSONNEL"];
  ...
  switch (header.textContent) {
    case "AUDITION DATE":
      ...
      dates = dates.map(function(dateDict) {
        return { start: new Date(dateDict.start), end: new Date(dateDict.end) };
      });
      desc += other.join("\\n");
      break;
    case "LOCATION":
      ...
      break;
    default:
      if (!others.includes(header.textContent)) break;
      if (desc != "") desc += "\\n";
      desc +=
        header.textContent +
        "\\n" +
